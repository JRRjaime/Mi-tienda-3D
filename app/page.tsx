...
import { User, Heart, Settings, Upload, LogOut } from "lucide-react"
            ...
            {!isAuthenticated ? (
              ...
              <DropdownMenu>
                <DropdownMenuTrigger asChild>
                  <Button
                    variant="ghost"
                    className="relative h-10 w-10 rounded-full border-2 border-white/30 hover:border-white/50"
                  >
                    <Avatar className="h-8 w-8">
                      <AvatarImage src={user?.avatar || "/placeholder.svg"} alt={user?.name} />
                      <AvatarFallback className="bg-gradient-to-r from-cyan-400 to-purple-500 text-white text-sm font-bold">
                        {user?.name
                          ?.split(" ")
                          .map((n) => n[0])
                          .join("")
                ...
                <DropdownMenuContent
                  className="w-64 bg-slate-900/95 backdrop-blur-md border-white/20"
                  align="end"
                  forceMount
                >
                  ...
                  <DropdownMenuSeparator className="bg-white/20" />
                  <DropdownMenuItem className="text-red-400 hover:bg-red-500/10 cursor-pointer" onClick={logout}>
